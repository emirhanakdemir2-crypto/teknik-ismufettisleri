...
- Kaynak logo: `public/brand/logo-source.jpeg` (TİMDER amblemi — fedora, büyüteç, onay işareti, müfettiş silueti).
- Üretim kullanımı: `src/components/brand/site-logo.tsx` inline SVG mark; kaynak PNG/JPG header’da ham kullanılmaz.
- Ana fikir korunur: müfettiş/inceleme sembolleri (şapka, büyüteç, onay); endüstriyel arka plan silüetleri ve metin halkası sadeleştirilir.
- Logo devlet kurumu amblemi, resmi mühür veya bakanlık logosu gibi görünmemeli.
- AI/SaaS gradient, neon, parlak mavi/mor palet kullanılmaz.
- Renkler site paletiyle uyumlu: `--navy` (zeytin), `--copper` / `--copper-muted` (bakır vurgu), `--header-on-primary` (kemik zemin).
- Varyantlar:
  - `full`: mark + dernek adı + “Müfettişe Sor” (desktop header)
  - `compact`: mark + dernek adı (mobil header)
  - `mark`: yalnızca ikon (dar alanlar, favicon benzeri kullanım)
- Küçük boyutta okunabilirlik: mark daire içinde, minimum ~32px; metin `ellipsis` ile taşmaz.
- İnceleme: `.cursor/skills/brand-logo-review/SKILL.md`
...
- `.cursor/skills/brand-logo-review/SKILL.md`
